...
MainWindow::MainWindow() {
	...
	rankButton = new QRadioButton("Page Rank");
	rankButton->setChecked(true);
	...
	radioButtonsFile->addWidget(rankButton);
	...
	connect(rankButton, SIGNAL(clicked()), this, SLOT(sortByRank()));
...
void MainWindow::setConfigFile(string config) {
	configFile = config;
}
...
void MainWindow::initializeSearchEng() {
	...
	PageParser* parser = &p;
	if(configFile.empty())
		engine.add_parse_no_config(parser);
	else
		engine.add_parse_from_config_file(configFile, parser);
...
void MainWindow::sortByRank() {
	if(fileListWidget->count() == 0)
		return;
	fileListWidget->clear();
	MySet<WebPage*> results;
	for(unsigned int i = 0; i <resultsVector.size(); i++) {
		results.insert(resultsVector[i]);
	}

	vector<WebPage*> pageRanks = engine.pageRank(&results);
	resultsVector.clear();
	for(unsigned int i = 0; i <pageRanks.size(); i++) {
		resultsVector.push_back(pageRanks[i]);
	}
